...
function renderLanding() {
  ...
  const subCount = sorted().filter(s => !s.soon).length;
  ...
  app.innerHTML = `<section class="landing"><div class="lnd-wrap">
    <nav class="lnd-nav"><div class="lnd-brand"><div class="lnd-mark">${BRAND_ICON}</div><div><div class="lnd-brand-name">Shot Deck</div><div class="lnd-brand-sub">SSC Study Hub · PWA</div></div></div>
    ...
    <div class="lnd-hero"><div class="lnd-pill"><b>${total} chapters</b> · ${subCount} subjects · installable app</div><h1 class="lnd-title">Master SSC,<br><span class="grad">one chapter at a time.</span></h1><p class="lnd-sub">Premium one-shot classes with a cinematic player, offline-ready PWA, progress tracking, and keyboard shortcuts — your entire SSC prep, beautifully organized.</p><div class="lnd-cta-row"><a class="lnd-cta" href="${rTarget}">${rLabel} ${svg(ICO.arrow,{sw:2.2})}</a><button class="lnd-install hidden" id="pwaInstall">${svg('<path d="M12 3v12M7 8l5-5 5 5"/><path d="M5 21h14"/>',{sw:2})} Install app</button><span class="lnd-cta-note">${done > 0 ? `${done} of ${total} chapters done` : `${total} chapters · ${subCount} subjects`}</span></div></div>
    <div class="lnd-stats"><div class="lnd-stat-card"><div class="n">${subCount}</div><div class="l">Subjects</div></div><div class="lnd-stat-card"><div class="n">${total}</div><div class="l">Chapters</div></div><div class="lnd-stat-card"><div class="n">${pct}%</div><div class="l">Your progress</div></div></div>
...
window.addEventListener('beforeinstallprompt', e => { e.preventDefault(); deferredInstall = e; showInstallBtn(); });
window.addEventListener('appinstalled', () => { deferredInstall = null; hideInstallBtn(); });
...
function initPwaInstall() {
  ...
  else if (window.matchMedia('(display-mode: standalone)').matches || navigator.standalone) hideInstallBtn();
  btn.onclick = async () => {
...
function drawList(sub) {
  $('hubList').innerHTML = sub.chapters.map(ch => {
    ...
    const pending = !cnt;
    const pill = pending
      ? `<span class="chap-pills"><span class="chap-pill soon">Soon</span></span>`
      : (cnt > 1 ? `<span class="chap-pills"><span class="chap-pill">${cnt} classes</span></span>` : '');
    const href = pending ? `#/${sub.key}` : `#/${sub.key}/${ch.n}`;
    return `<a class="chap ${w?'done':''} ${pending?'pending':''}" href="${href}"><div class="chap-num">${String(ch.n).padStart(2,'0')}</div><div class="chap-body"><div class="chap-title">${ch.title}</div><div class="chap-desc">${ch.desc}</div></div>${pill}<span class="chap-go">${svg(ICO.arrow,{sw:2})}</span>${pending ? '' : `<button class="mark-btn ${w?'done':''}" data-n="${ch.n}">${w?svg(ICO.check,{sw:2.4})+' Completed':svg(ICO.circle,{sw:2})+' Mark'}</button>`}</a>`;
  }).join('');
  document.querySelectorAll('#hubList .mark-btn').forEach(b => b.onclick = e => {
    e.preventDefault(); e.stopPropagation();
    const on = toggleWatched(sub.key, b.dataset.n);
    b.closest('.chap')?.classList.toggle('done', on);
    b.classList.toggle('done', on);
    b.innerHTML = on ? svg(ICO.check, { sw: 2.4 }) + ' Completed' : svg(ICO.circle, { sw: 2 }) + ' Mark';
    const pct = subjectPct(sub);
    const fill = document.querySelector('.hub-prog-fill'); if (fill) fill.style.width = pct + '%';
    const v = document.querySelector('.hub-prog-top .v'); if (v) v.textContent = pct + '%';
    buildNav($('subjects'), sub.key, null);
  });
}

function ph(sub, msg) {
  const text = msg || `${sub.bengali} — coming soon.`;
  return `<div class="placeholder"><div class="ph" style="--c:${sub.color}"><div class="ph-ico">${svg('<circle cx="12" cy="12" r="9"/>',{sw:1.7})}</div><div class="ph-title">${sub.name}</div><p class="ph-sub">${text}</p></div></div>`;
}

function phChapter(sub, ch) {
  return `<div class="placeholder"><div class="ph" style="--c:${sub.color}"><div class="ph-ico">${svg(sub.icon,{sw:1.7})}</div><div class="ph-title">${String(ch.n).padStart(2,'0')}. ${ch.title}</div><p class="ph-sub">Class video coming soon — check back later.</p><div class="ph-tags"><span class="ph-tag"><span class="d"></span>${sub.name}</span><span class="ph-tag">Chapter ${String(ch.n).padStart(2,'0')}</span></div></div></div>`;
}

/* ─── CHAPTER ─── */
let curCh = null, curSub = null, curIdx = null, curClasses = [];
const SPEEDS = [0.5, 0.75, 1, 1.25, 1.5, 1.75, 2];
const QLABELS = { auto: 'Auto', highres: '4320p', hd2160: '2160p', hd1440: '1440p', hd1080: '1080p', hd720: '720p', large: '480p', medium: '360p', small: '240p', tiny: '144p' };
const QORDER = ['highres', 'hd2160', 'hd1440', 'hd1080', 'hd720', 'large', 'medium', 'small', 'tiny'];
const VOLSVG = {
...
function renderChapter(key, n) {
  ...
  if (!curClasses.length) { ma.innerHTML = phChapter(sub, ch); return; }
...
function drawLesson() {
  ...
  $('markBtn').onclick = () => {
    const on = toggleWatched(curSub.key, curCh.n);
    const b = $('markBtn');
    b.classList.toggle('done', on);
    b.innerHTML = on ? svg(ICO.check, { sw: 2.4 }) + ' Completed' : svg(ICO.circle, { sw: 2 }) + ' Mark complete';
    buildNav($('subjects'), curSub.key, curCh.n);
  };
}
...
function startPlayer(videoId, list) {
  curVideoId = videoId; curList = list; fellBack = false; userQuality = 'auto';
  loadYTApi(function create() {
    ...
    const opts = { width: '100%', height: '100%', playerVars: pv, videoId: videoId || undefined, host: 'https://www.youtube-nocookie.com', events: { onReady, onStateChange, onPlaybackRateChange: () => updateSpeed(), onPlaybackQualityChange: () => { buildQuality(); updateQualityBtn(); }, onError: onErr } };
...
let userQuality = 'auto';
function buildQuality() {
  const menu = $('vpQualityMenu'); if (!menu) return;
  let avail = [];
  if (ytPlayer && !fellBack) { try { avail = ytPlayer.getAvailableQualityLevels?.() || []; } catch {} }
  const levels = ['auto', ...QORDER.filter(q => avail.includes(q))];
  // Only 'auto' available (levels not populated until playback) → keep a sensible default menu.
  const list = levels.length > 1 ? levels : ['auto', 'hd1080', 'hd720', 'large', 'medium', 'small'];
  menu.innerHTML = list.map(q => `<button class="vp-menu-item ${q === userQuality ? 'active' : ''}" data-q="${q}"><span>${QLABELS[q] || q}</span>${svg(ICO.check, { sw: 2.4, cls: 'vp-menu-check', fill: 'none' })}</button>`).join('');
  menu.querySelectorAll('.vp-menu-item').forEach(b => b.onclick = e => {
    e.stopPropagation(); const q = b.dataset.q; userQuality = q;
    if (ytPlayer && !fellBack) { try { ytPlayer.setPlaybackQuality(q); if (q !== 'auto') ytPlayer.setPlaybackQualityRange?.(q, q); } catch {} }
    updateQualityBtn(); menu.classList.remove('open'); $('vpQuality')?.classList.remove('active');
  });
}
function updateQualityBtn() {
  const b = $('vpQuality'); if (!b) return;
  let shown = userQuality;
  if (userQuality === 'auto' && ytPlayer && !fellBack) { try { const q = ytPlayer.getPlaybackQuality?.(); if (q && q !== 'unknown') shown = 'auto'; } catch {} }
  b.innerHTML = `${QLABELS[shown] || 'Auto'} ${svg(ICO.chev, { sw: 2 })}`;
  $('vpQualityMenu')?.querySelectorAll('.vp-menu-item').forEach(x => x.classList.toggle('active', x.dataset.q === userQuality));
}
...
function bindController() {
  ...
  const sb = $('vpSpeed'), sm = $('vpSpeedMenu'), qb = $('vpQuality'), qm = $('vpQualityMenu');
  sb.onclick = e => { e.stopPropagation(); qm?.classList.remove('open'); qb?.classList.remove('active'); sm.classList.toggle('open'); sb.classList.toggle('active', sm.classList.contains('open')); };
  if (qb && qm) qb.onclick = e => { e.stopPropagation(); sm.classList.remove('open'); sb.classList.remove('active'); buildQuality(); qm.classList.toggle('open'); qb.classList.toggle('active', qm.classList.contains('open')); };
  const closeMenus = e => { if (!e.target.closest('.vp-menu-wrap')) { sm.classList.remove('open'); sb.classList.remove('active'); qm?.classList.remove('open'); qb?.classList.remove('active'); } };
    ...
    navigator.serviceWorker.register('/sw.js').then(reg => {
      reg.addEventListener('updatefound', () => {
        ...
        nw.addEventListener('statechange', () => {
          if (nw.state === 'installed' && navigator.serviceWorker.controller) showSwUpdate();
        });
      });
    }).catch(() => {});
  });
...
function showSwUpdate() {
  ...
  bar.innerHTML = `<span>New version available</span><button type="button" id="swReload">Refresh</button>`;
...
function initOfflineBadge() {
  ...
  const sync = () => badge.classList.toggle('show', !navigator.onLine);
